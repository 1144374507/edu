...
<template>
    <div>
        <div style="margin-bottom: 28px">
            ...
            <template v-else>
                <span style="font-size: 16px" class="__title">修改配置</span>
                <!-- <el-button
                    type="text"
                    @click="goEditCli"
                ...
                <div>
                    ...
                    >
                        <el-form-item label="设备名称" prop="hostName">
                            <el-input
                                :id="
                                    'qa-test-Ops-device-option-modifyConfig-deviceName-' +
                                    form1.hostName
                                "
                                v-model="form1.hostName"
                                style="width: 208px"
                            ></el-input>
                        </el-form-item>
                        <el-form-item label="设备型号">
                            <el-input
                                v-model="payload.device_model_id"
                                style="width: 208px"
                                disabled
                            ></el-input>
                        </el-form-item>
                        <el-form-item label="区域名称">
                            <areaSelect
                                :id="
                                    'qa-test-Ops-device-option-modifyConfig-areaName-select-' +
                                    form1.areaId
                                "
                        ...
                        <el-form-item label="设备位置">
                            <el-input
                                id="qa-test-Ops-device-option-modifyConfig-deviceLocation"
                    ...
                    <div style="text-align: right; margin-top: 35px">
                        ...
                        <el-button type="primary" @click="handleSaveBtnClick"
                            >保存</el-button
                        >
                    </div>
                </div>
...
import addTelent from "@inap-vlan/components/deviceManage/deviceTemplate/addTelent.vue";
import addSnmp from "@inap-vlan/components/deviceManage/deviceTemplate/addSnmp.vue";
...
export default {
    ...
    data() {
        return {
            ...
            form1: {
                deviceId: this.payload.id,
                serialNo: this.payload.serial_number,
                hostName: this.payload.name,
                areaId: this.payload.areaId,
            ...
            form2: {
                ...
                DEVICE_FIBER_ENABLE: false,
                DEVICE_SERVICE_ENCRYPT: false,
                logServerEnabled: false,
                DEVICE_LOGIN_SERVER_HOST: "",
                DEVICE_LOGIN_SERVER_PORT: "514",
    ...
    computed: {
        isEdit() {
            return this.payload && this.payload.devCliTmplId;
        },
    },
    methods: {
        getTempateData() {
            ...
            this.$axios
                .get(
                    `/api/vlan/customConfigTemplate/get/${this.payload.devCliTmplId}`
                )
                .then(({ data: { success, result, message } }) => {
                    if (success) {
                        ...
                        // 可能存在设备端口数与模版端口数不一致的情况，所以这里做一次校正
                        this.onPortNumChange(this.payload.portNum - this.payload.fibrePortNums);
                    }
                })
                .finally(() => {
            ...
            this.$refs.form2.validate((valid) => {
                ...
                let message =
                    this.isEdit && (this.payload.deviceNum || this.priginConfigTemplate.telnetTmplId !== this.form1.telnetTmplId)
                        ? `保存后，将下发配置给该设备。本次修订会导致设备重启，请在业务空闲时段操作。`
                        : "";
        ...
        save() {
            const config = this.genConfig();
            let url = "/api/vlan/customConfigTemplate/device/save";
            config.id = this.payload.devCliTmplId;
            this.$axios
                .post(url, config)
                .then(({ data: { success, data, message } }) => {
                    if (success) {
    ...
    created() {
        ...
        if (this.isEdit) {
            ...
            this.form1.portNum = this.payload.portNum;
            this.onPortNumChange(this.payload.portNum - this.payload.fibrePortNums); // 忽略光口
            this.form2.DEVICE_FIBER_ENABLE = true;
            this.form2.DEVICE_SERVICE_ENCRYPT = true;
            this.form2.DEVICE_MAC_NOTIFICATION_PERIOD = "60";
        }
    },
};
</script>

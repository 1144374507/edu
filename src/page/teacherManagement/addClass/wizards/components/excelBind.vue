...
export default {
    ...
    data() {
        return {
            ...
            uploadUrl:
                "/api/vlan/zeroConfig/importPreview?token=" +
                tokenObj.getToken(),
            accept: ".xls,.xlsx",
            fileList: [],
            uploadDoing: false,
    ...
    methods: {
        ...
        downLoad() {
            rjDownload.download(
                "/api/vlan/zeroConfig/download?params=" +
        ...
        handleError(err, file) {
            if (err.message) {
                const entity = JSON.parse(err.message);
                this.uploadDoing = false;
                this.$alert(
                    entity.message.replace("服务器异常:", "") || "导入失败",
                    "导入失败",
                    {
                        confirmButtonText: "关闭",
                        type: "error",
        ...
        submitBind() {
            ...
            this.$axios
                .put("/api/vlan/zeroConfig/importZeroConfig", this.sourceResult)
